...
# ── App Info ──────────────────────────────────────────────────────────────────
APP_NAME = "Game Audio Translator"
CONFIG_PATH = Path(__file__).parent / "game_translate_config.json"
...
# ── Config ────────────────────────────────────────────────────────────────────
def load_config():
    defaults = {"api_key": "", "language": "English (en)", "voice": "Puck (Upbeat)", "mic": "cable output",
                "output": "turtle beach", "echo": True, "volume": 1.0, "in_volume": 1.0, "monitor_mic": False}
    try:
...
class TranslationEngine:
    def __init__(self, api_key, target_lang, voice_name, echo, mic_kw, out_kw,
                 transcript_q, level_q, status_cb, volume_ref, in_volume_ref, monitor_ref):
...
class SettingsDialog(ctk.CTkToplevel):
    def __init__(self, parent, cfg, on_save):
        ...
        # Mic keyword
        ctk.CTkLabel(self, text="Microphone/Desktop Audio (search keyword)", font=(FONT, 13, "bold"),
                     text_color=TEXT).pack(anchor="w", **pad)
        ...
        ctk.CTkLabel(self, text="Output Device (search keyword)", font=(FONT, 13, "bold"),
                     text_color=TEXT).pack(anchor="w", **pad)
        self.out_var = ctk.StringVar(value=cfg.get("output", "turtle beach"))
...
class App(ctk.CTk):
    ...
    def _build_ui(self):
        ...
        # Bottom Controls
        bottom_frame = ctk.CTkFrame(self, fg_color="transparent", height=180)
        ...
        self.lang_btn = ctk.CTkButton(
            sel_bar, text=self.cfg.get("language", "English (en)"),
        ...
        vol_frame.pack(fill="x", pady=(0, 8))
        
        ctk.CTkLabel(vol_frame, text="\U0001F3A4", font=(FONT, 16), text_color=TEXT_DIM).pack(side="left", padx=(0, 8))
        ...
        self.in_vol_label = ctk.CTkLabel(vol_frame, text=f"{int(self.in_vol_var.get() * 100)}%", font=(FONT, 12), text_color=TEXT_MUT, width=40)
        self.in_vol_label.pack(side="right", padx=(8, 0))

        # Output Volume slider
        out_vol_frame = ctk.CTkFrame(bottom_frame, fg_color="transparent")
        out_vol_frame.pack(fill="x", pady=(0, 16))
        
        ctk.CTkLabel(out_vol_frame, text="\U0001F508", font=(FONT, 16), text_color=TEXT_DIM).pack(side="left", padx=(0, 8))
        self.vol_var = ctk.DoubleVar(value=self.cfg.get("volume", 1.0))
        self.vol_slider = ctk.CTkSlider(out_vol_frame, variable=self.vol_var, from_=0.0, to=2.0, command=self._on_vol_change)
        self.vol_slider.pack(side="left", fill="x", expand=True)
        self.vol_label = ctk.CTkLabel(out_vol_frame, text=f"{int(self.vol_var.get() * 100)}%", font=(FONT, 12), text_color=TEXT_MUT, width=40)
    ...
    def _start(self):
        ...
        self.engine = TranslationEngine(
            ...
            out_kw=self.cfg.get("output", "turtle beach"),
